...
export default class LearnMore extends Component {
  ...
  render () {
    ...
    if (not_enough_text_to_truncate) {
      return <Text>{expanded_text_to_display}more</Text>;
    }

    if (this.state.readMore) {
      return <Text tabIndex="0" numberOfLines={num_of_lines} ellipsizeMode='tail' onPress={this.showMore} onKeyDown={this.onKeyDown.bind(this)}>
          {text_to_display}
          <Text tabIndex="0" style={{color: '#48BBEC'}}>{show_more_text}</Text>
        </Text>;
    } else {
      return <Text tabIndex="0"> {expanded_text_to_display}&nbsp;&nbsp;
        <Text tabIndex="0" style={{color: '#48BBEC'}}
          onPress={on_click}
