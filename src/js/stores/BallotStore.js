...
class BallotStore extends FluxMapStore {
  ...
  get ballot_properties () {
    ...
    // console.log("ballot_properties, props: ", props);
  ...
  get bookmarks () {
    ...
    ballot.forEach( ballot_item => {
      if (BookmarkStore.getBookmark(ballot_item.we_vote_id)){ // item is bookmarked
        bookmarks.push(ballot_item);
      }
      if (ballot_item.candidate_list) {
        ballot_item.candidate_list.forEach(candidate => {
          if (candidate) {
            if (BookmarkStore.getBookmark(candidate.we_vote_id)) {
    ...
    switch (action.type) {
      ...
      case "voterBallotItemsRetrieve":
        ...
        if (google_civic_election_id !== 0) {
          ...
          state.ballots = assign({}, state.ballots, newBallot);
        }
        return {
          ...state
        };
      ...
      case "voterBallotListRetrieve":
        ...
        state.ballot_election_list = ballot_election_list;
        return {
          ...state
        };
      ...
      case "voterAddressSave":
        if (action.res.status === "SIMPLE_ADDRESS_SAVE") {
          ...
          if (google_civic_election_id !== 0) {
            ...
            state.ballots = assign({}, state.ballots, newBallot);
            state.ballotCaveat = ballotCaveat;
          }
        }
        return {
          ...state
